...
public class UImanager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // opennewHideSocore = gameObject.transform.Find("newsocore").gameObject;
        ...
        btnShowListChar.onClick.AddListener(() =>
        {
            FindObjectOfType<Showandhide>().backcameratobehigh();
            FindObjectOfType<Soundmanager>().OpenSoundClic();
            FindObjectOfType<Camerafolow>().ShowShop3D();
        });
    ...
    public void SetingInStart()
    {
        ...
        coinmuving = 0;
        if (!camerafolow)
            camerafolow = FindAnyObjectByType<Camerafolow>().gameObject;
        Debug.Log("cameraFollow: " + camerafolow);
        amin = FindAnyObjectByType<Camerafolow>().animator;
        Debug.Log("amin: " + amin);
        uimanager = this;
        // tắt các timer item
    ...
    public void Again()
    {
        ...
        Debug.Log("amin: " + amin + ", camerafolow: " + camerafolow);
        uimanager = this;
        // tắt các timer item
        panellost.gameObject.SetActive(false);
        panelseleckey.gameObject.SetActive(false);
    ...
    public void playAgain()
    {
        if (calll)
        {
            ...
            if (!showPlayeronlost)
            {
                showPlayeronlost = FindObjectOfType<Camerafolow>().cameraLost;
            }
            ...
            FindObjectOfType<goodCPU>().GetStartrotay(true);
    ...
    IEnumerator Playdelay()
    {
        if (!amin)
        {
            amin = FindObjectOfType<Camerafolow>().GetComponent<Animator>();
        }
        Debug.Log("amin: " + amin);
    ...
    void LoaDingcharacterOnshowNewHideSocore()
    {
        ...
        if (!opennewHideSocore)
        {
            opennewHideSocore = FindObjectOfType<Camerafolow>().newsocore;
        }
        opennewHideSocore.gameObject.SetActive(true);
        if (!Lischaracter)
        {
            Lischaracter = FindObjectOfType<Camerafolow>().listCharNewScore;
        }
    ...
    public IEnumerator refreshKeyAndCoinLose() {
         yield return new WaitForSeconds(1);
          yourkeytxt.text = managerdata.manager.getkey().ToString();
        yourcoin.text = managerdata.manager.Getcoin().ToString();
    }
